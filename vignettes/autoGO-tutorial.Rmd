...
groups <- data.frame(sample=c("Pat_1","Pat_2","Pat_3","Pat_4","Pat_5","Pat_6"), group=c("CTRL", "CTRL", "TREAT_A", "TREAT_A", "TREAT_B", "TREAT_B"))
groups
```
...
```{r, echo=FALSE, eval=TRUE}
comparisons <- data.frame(treatment = c("TREAT_A","TREAT_B","TREAT_A"), control = c("CTRL", "CTRL", "TREAT_B"))
comparisons
```
...
* *res*: it accepts both a file (.tsv, .csv, tab-separated .txt) or a dataframe (see example below). It is necessary that the columns _genes_, _log2FoldChange_ and _padj_ are present.
...
## Reading gene_lists
*read_gene_list.R* is the function employed to read all the gene lists on which the enrichment will then be performed.

```{r}
#DO NOT RUN FOR SINGLE LIST

##TO DO: DEBUG WHERE FILES READGENELIST
gene_lists_path = "./results/H460.2D_vs_H460.3D.2p/filtered_DE_thFC0_thPval0.05/down_genes"
list_of_genes <- read_gene_list(gene_lists_path = gene_lists_path,
               log2FC_threshold = 0, 
               padj_threshold = 0.05, 
               which_list = "down_genes",#,"down_genes","up_down_genes","everything"),
               from_DE_analysis = T,
               files_format = NULL)
names(list_of_genes)
```
...
```{r}
gene_list <- list_of_genes$`./results/H460.2D_vs_H460.3D.2p/filtered_DE_thFC0_thPval0.05/down_genes/down_genes_list_H460.2D_vs_H460.3D.2p_thFC0_thPval0.05`
autoGO(list_of_genes = gene_list, 
...
* *list_of_genes*: it can be a list of dataframe (i.e. the output of *read_gene_list.R*), a vector or a dataframe containing the list of the genes or the path to the file .txt where the gene list is contained.
...
* *ensembl*: default False, set TRUE if the provided gene list contains Ensembl IDs. A conversion to HGNC will be performed.

* *excel*: default FALSE, set TRUE if you want to save tables in .xlsx format.

* *where_results* : specify the folder in which you want to save the outputs. Default is "./". Note: if you are working with RNotebook the default working directory, if not specified, is the folder in which the .Rmd file is saved.

* *outfolder* : the name with which to call the folder to save the outputs. Default is: "results/". NOTE: please add "/" at the end.
_
* *my_autoGO_dir*: where have you cloned the auto-go repository, default is your home directory "~/"


If there are more lists on which to perform the enrichment analysis it is suggested to proceed with the following 
approach:

```{r}
gene_list <- read_gene_list(gene_lists_path = "./results/", log2FC_threshold = 0, from_DE_analysis = T, which_list = "down_genes")
names(gene_list)
lapply(names(gene_list), function (i) autoGO(gene_list[[i]], my_comparison = i))
```


```{r}
#gene_list <- read_gene_list(gene_lists_path = "./results/", from_DE_analysis = T, which_list = "down_genes")
#lapply(names(gene_list), function (i) autoGO(gene_list[[i]], my_comparison = i))
```
...
```{r}
DO NOT RUN FOR SINGLE ENRICHMENT TABLE
read_enrich_tables(where_results = "./", 
                   outfolder = "results/",
                   log2FC_threshold = 0, 
                   padj_threshold = 0.05, 
                   which_list = C("up_genes", "down_genes", "up_down_genes", "everything"), 
...
```{r}
#TO DO: test read_enrich_table
enrich_table_path <- "./results/H460.2D_vs_H460.3D.SM/"
enrich_tables <- read_enrich_tables(enrich_table_path = enrich_table_path,log2FC_threshold = 0,
                   padj_threshold = 0.05, which_list = "down_genes", from_DE_analysis = T,
                   where_files = NULL, files_format = NULL)
names(enrich_tables)
```
...
barplotGO(enrich_table = enrich_table,  
          my_comparison = "H460.2D_vs_H460.3D.SM", 
          where_results = "./", 
          outfolder = "results/")
```
...
* *enrich_table*: dataframe containing the enrichment results or a path to your .tsv file containing the enrichment results. It is necessary that it contains the columns *Term* and *Adjusted.P.Value*.
...
If there are more lists on which the enrichment has been performed it is suggested to proceed with the following approach:
```{r}
#TO DO: test read_enrich_table
enrich_list <- read_enrich_tables(enrich_table_path = "results/", which_list = "down_genes")
names(enrich_list)
lapply(names(enrich_list), function (i) {barplotGO(enrich_list[[i]], my_comparison = i)})
...
```{r}
TO DO: test read_enrich_table
filename <- enrich_table$"./results/H460.2D_vs_H460.3D.2p/filtered_DE_thFC0_thPval0.05/down_genes/enrichment_tables/GO_Biological_Process_2021.tsv"
lolliGO(enrich_table = filename,  
          my_comparison = "H460.2D_vs_H460.3D.2p", 
          where_results = "./", 
          outfolder = "results/")
```
...
* *enrich_table*: dataframe containing the enrichment results or a path to your .tsv file containing the enrichment results. It is necessary that it contains the columns *Term* and *Adjusted.P.Value*. It has to contain also the column *Overlap* which indicates the number of genes in our list enriched over the total genes of that term (format n/N).
...
If there are more lists on which the enrichment has been performed it is suggested to proceed with the following approach:
```{r}
enrich_list <- read_enrich_tables(which_list = "everything")
lapply(names(enrich_list), function (i) lolliGO(enrich_list[[i]], my_comparison = i))
```
