...
library(devtools)
#install_github("mpallocc/auto-go", ref = "add-tutorial-vignette", force = T)
install(".")
library(autoGO)
```
...
It is also possible to call the function in a for loop in order to realize volcano plots with different threshold, for example:
```{r}
for (th in c(1,2,3)) {
  lapply(names(res_lists), function (i) volcanoplot(res_lists[[i]], my_comparison = i, log2FC_thresh = th))
}
```
...
```{r}
autoGO(list_of_genes = list_of_genes$down_genes_list_H460.2D_vs_H460.3D.2p_thFC0_thPval0.05, 
...
If there are more lists on which to perform the enrichment analysis it is suggested to proceed with the following approach:
```{r}
gene_list <- read_gene_list(which_list = "down_genes")
...
```{r}
filename <- "./results/H460.2D_vs_H460.3D.2p/filtered_DE_thFC0_thPval0.05/down_genes/enrichment_tables/GO_Biological_Process_2021.tsv"
barplotGO(enrich_table = filename,  
          my_comparison = "H460.2D_vs_H460.3D.2p", 
          where_results = "./", 
          outfolder = "results/")
```
