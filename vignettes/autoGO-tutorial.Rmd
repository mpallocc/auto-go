...
* *outfolder* : the name of the output folder. Default is: "./results".

* *del_csv* : default ",". The delimiter of the counts file.

#### Workflow:
The DESeq2 workflow is employed. Raw counts are rounded. A prefiltering is applied, if the user choose to, in order to remove all genes having sum along the sample less than 10.

The normalized data matrix is stored in the *outfolder*. Subfolders will be generated with the name of the comparisons in the related file furnished by the user. Here there will be saved the differential analysis tables (normalized counts and vst). Inside those, other subfolders will be generated based on the threshold of the log2FC and of the Adj. P. Value where the user can find the lists and tables of the filtered genes and downstream analysis results.


## Filtering DESeq2 results

It allows to filter the Differential Expression Analysis results with the new filters directly from the output table "*_allres.tsv" without repeating the whole analysis.

```{r}
filtering_DE(padj_threshold = 0.05,
             log2FC_threshold = 1,
             where_results = "./", #FIXME: cut where_results (in una singola linea di codice)
             outfolder = "results/",
             save_excel = F)
```
...
* *DE_results*: it accepts both a file (.tsv, .csv, tab-separated .txt) or a dataframe. It is necessary that the columns *genes*, *log2FoldChange* and *padj* are present.

* *padj_threshold* and *log2FC_threshold*: Defaults are padj_threshold = 0.05, log2FC_threshold = 0

* *my_comparison*: the comparison the user would like to plot (taken from the comparison file: control_vs_treatment, a_vs_b)
...
If the user had more conditions in the differential analysis he probably produced many comparisons, so it is possible to apply the following structure to realize the volcano plots for all the comparisons of the analysis:
...
* *where_files* : dafult NULL, when from_autoGO = T it is mandatory to provide a path to specify where the list of genes are.

* *files_format* : default NULL, when from_autoGO = T it is mandatory to provide the extension of the list of genes you want to upload.
...
heatmapGO(lib = "GO_Biological_Process_2021", 
          where_results = "./",
          outfolder = "results/", 
          log2FC_threshold = 0,
          padj_threshold = 0.05, 
