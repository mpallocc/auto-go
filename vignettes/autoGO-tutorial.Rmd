...
Auto-GO allows the user to visualize the enrichment results with different kind of plot produced in an automated way. The barplot and the lolliplot of the most enriched terms of each list and the heatmap of the most enriched terms along all the enriched lists.
...
knitr::include_graphics("./results/H460.2D_vs_H460.3D.2p/filtered_DE_thFC0_thPval0.05/down_genes/enrichment_plots/barplotGO.png")
```
...
* *title* : Default NULL. When enrichment tables are not from autoGO and thus from read_enrich_tables, the user must title and subtitle of the plot as 2-element character vector: c("This is the title", "this is the subtitle")

* *outfolder* : the name of the output folder. #FIXME: EVEN IN THIS CASE? Ignored if enrich_tables is a list. Default is: "./results". (NOTE: The outfolder chosen by the user will be the parent folder of the results. Inside this, the subfolders will be organized following the autoGO structure in order to keep the results of the different analysis ordered.)

* *from_autoGO* : set to FALSE if the enrichment tables you want to use are not from a differential expression analysis. Default is TRUE.
...
```{r, echo=FALSE, out.width = '98%',  eval=TRUE} 
knitr::include_graphics("./results/H460.2D_vs_H460.3D.2p/filtered_DE_thFC0_thPval0.05/down_genes/enrichment_plots/lolliGO.png")
```
...
heatmapGO(db = "GO_Biological_Process_2021",
          ...
          log2FC_threshold = 0,
          padj_threshold = 0.05,
          min_term_per_row = 1, #FIXME: 1 vuol dire >1 e quindi 2 confronti significativi per termine? then default = 1
          which_list = "down_genes")
```
...
knitr::include_graphics("./results/ComparisonHeatmap/down_genesGO_Biological_Process_2021_min2.png")
```
